; // bundle: page___b15e980a85f7934465d090525413733c_m
...
Roblox.ItemPurchase = function(purchaseConfirmationCallback, purchaseCallback, shouldShowFeedbackBanner) {
    ...
    function purchaseItem(elem, CssClass) {
        ...
        var gamePassesServiceUrl = Roblox.EnvironmentUrls.apiGatewayUrl + "/game-passes/v1/game-passes/{productID}/purchase".replace('{productID}', productID);
        if (purchaseCallback) {
    ...
    function startPaymentFlowAndSendModalEvent(targetData) {
        if (paymentFlowAnalyticsService && targetData) {
            paymentFlowAnalyticsService.startRobuxUpsellFlow(targetData.assetType, !!targetData.userassetId, false, false);
            paymentFlowAnalyticsService.sendUserPurchaseFlowEvent(paymentFlowAnalyticsService.ENUM_TRIGGERING_CONTEXT.WEB_CATALOG_ROBUX_UPSELL, true, paymentFlowAnalyticsService.ENUM_VIEW_NAME.ROBUX_UPSELL, paymentFlowAnalyticsService.ENUM_PURCHASE_EVENT_TYPE.VIEW_SHOWN);
        }
    }

    function onInsufficientFundsViewCancel() {
        if (paymentFlowAnalyticsService) {
            paymentFlowAnalyticsService.sendUserPurchaseFlowEvent(paymentFlowAnalyticsService.ENUM_TRIGGERING_CONTEXT.WEB_CATALOG_ROBUX_UPSELL, true, paymentFlowAnalyticsService.ENUM_VIEW_NAME.ROBUX_UPSELL, paymentFlowAnalyticsService.ENUM_PURCHASE_EVENT_TYPE.USER_INPUT, paymentFlowAnalyticsService.ENUM_VIEW_MESSAGE.CANCEL);
        }
    }
    ...
    function openErrorView(errorObj) {
        if (errorObj.showDivID === "TransactionFailureView") {
            ...
            if (Roblox.ItemPurchaseUpsellService && Roblox.ItemPurchaseUpsellService.showExceedLargestInsufficientRobuxModal) {
                Roblox.ItemPurchaseUpsellService.showExceedLargestInsufficientRobuxModal(errorObj.shortfallPrice, errorObj.targetData, function() {
...
Roblox.define('Widgets.ItemImage', [], function() {
            ...
            $.getJSON(config.endpoint, {
                ...
                return function(results) {
                    ...
                    for (var i = 0; i < results.length; i++) {
                        ...
                        var img = $('<img>').attr('title', data.name).attr('alt', data.name).attr('border', 0).addClass('original-image modal-thumb');
    ...
    function loadV2(element, assetName) {
        ...
        var imageSizeNum = imageSizeLookup[element.attr('data-image-size') || 'large'];
        var imageSizeString = imageSizeNum + 'x' + imageSizeNum;
        var apiEndpoint = Roblox.Endpoints.getAbsoluteUrl(Roblox.EnvironmentUrls.thumbnailsApi + "/v1/assets?assetIds=" + assetId + "&size=" + imageSizeString + "&format=webp");
        $.get(apiEndpoint, function(result) {
            ...
            imageContainer.html(innerDiv);
            imageContainer = innerDiv;
            var img = $('<img>').attr('title', assetName).attr('alt', assetName).attr('border', 0).addClass('original-image modal-thumb');
            img.load(function(innerDiv, element) {
                return function() {
                    innerDiv.width(element.width);
                    innerDiv.height(element.height);
                };
...
Roblox.GameDetailReferral.AppendUrl = function(element, listSelector, linkElement) {
    ...
    newUrl += "LocalTimestamp=" + new Date().toISOString() + "&TotalInSort=" + total;
...
$(function() {
    ...
    $('#recently-visited-places').on(events, '.game-card', function() {
        Roblox.GameDetailReferral.AppendUrl($(this).find(cardLinkSelector), '#recently-visited-places .list-item');
    });
    $('#my-favorites-games').on(events, '.game-card', function() {
        Roblox.GameDetailReferral.AppendUrl($(this).find(cardLinkSelector), '#my-favorites-games .game-card');
    });
    $('#friend-activity').on(events, '.game-card', function() {
        Roblox.GameDetailReferral.AppendUrl($(this).find(cardLinkSelector), '#friend-activity .game-card');
    });
    ...
    $('#GamesListsContainer').on(events, '.game-card', function() {
        ...
        Roblox.GameDetailReferral.AppendUrl($(this).find(cardLinkSelector), new Array(total));
    });
    $('#my-recommended-games').on(events, '.game-card', function() {
        Roblox.GameDetailReferral.AppendUrl($(this).find(cardLinkSelector), '#my-recommended-games .game-card');
    });
    $('#HomeContainer #FeaturedGamesContainer').on(events, '.item-place a', function() {
    ...
    if (typeof RobloxBadges.initRobloxBadgesFrameworkAgnostic !== "undefined") {
        try {
            ...
            if (typeof Roblox.EventStream !== "undefined") {
                Roblox.EventStream.SendEvent("RobloxBadgesInitFailure", "WebPlatformJsCaughtError", {});
            }
        }
    }
});
    ...
    toggleLink.click(function() {
        if (toggleLink.data("isguest").toLowerCase() === "true") {
            ...
            $.post(gamesApiUrl, {
                ...
                if (Roblox.BootstrapWidgets && Roblox.BootstrapWidgets.ToggleSystemMessage && warningElement.length) {
                    Roblox.BootstrapWidgets.ToggleSystemMessage(warningElement, errorSlideDown, errorDuration, langResources["Error.Favorite"]);
                }
            });
        }
    });
